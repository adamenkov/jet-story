Original game:
--------------

Full title: Jet-Story

Year of release: 1992

Publisher: Ultrasoft (Slovakia)

Authors: Cybexlab Software (Miroslav Fidler), Fuxoft (Frantisek Fuka)

Publication: Public domain



PC remake:
----------

Author of code: Evgeny Adamenkov (engine code inspired by book "Advanced 2D Game Development" by Jonathan S. Harbour, 2009)

Assets: Extracted from the original game using Spectaculator (Copyright (C) 2001-2008 Jonathan Needle)

Year of release: 2012

The game uses FMOD by Firelight Technologies
